...
class ESRGAN_EESN_FRCNN_Model(BaseModel):
    def __init__(self, config, device):
        ...
        self.netG.load_state_dict(torch.load('../saved/pretrained_models_EESRGAN_FRCNN/9360_G.pth'), strict=False)
        self.netG = self.netG.to(self.device)
        self.netG = DataParallel(self.netG)

        #descriminator
        self.netD = model.Discriminator_VGG_128(in_nc=self.configD['in_nc'], nf=self.configD['nf'])
        self.netG.load_state_dict(torch.load('../saved/pretrained_models_EESRGAN_FRCNN/9360_D.pth'), strict=False)
        ...
        # self.netG.load_state_dict(torch.load('../saved/pretrained_models_EESRGAN_FRCNN/9360_FRCNN.pth'))

        ###  Mask RCNN model ############
        self.netFRCNN=torchvision.models.detection.maskrcnn_resnet50_fpn(pretrained=True)
    ...
    def optimize_parameters(self, step):
        ...
        print('TARANWANTSTOCHECK',self.var_L.shape)
        # removed ,_ in the code below
