...
# sys.path.insert(0, "/raid/ai22mtech13004/EESRGAN/CannyEdgePytorch")
# from CannyEdgePytorch.canny import canny
...
class Discriminator_VGG_128(nn.Module):
    def __init__(self, in_nc, nf):
        ...
        # [1, 1024, 7, 7]
        ## New Layer added
        self.l1 = nn.Linear(1024 * 7 * 7, 1000)
    ...
    def forward(self, x):
        ...
        fea = self.lrelu(self.l1(fea))  # Modified according to new layers
        out = self.l2(fea)
        return out
...
class EESN(nn.Module):
  def __init__(self, filter_name,  filter_size):
  ...
  def forward(self, x):
    ######### CHANGE is HERE for FILTERS USED ###########
    if self.filter_name == "laplacian": 
        x_lap = kornia.laplacian(x, self.filter_size) # see kornia laplacian kernel
    elif self.filter_name == "sobel":
        x_lap = kornia.sobel(x) # see kornia sobel kernel
    elif self.filter_name == "spatial_gradient":
        x_lap_1 = kornia.filters.spatial_gradient(x)[:,:,0,:,:]
        x_lap_2 = kornia.filters.spatial_gradient(x)[:,:,1,:,:]
        x_lap = torch.sqrt(x_lap_1**2 + x_lap_2**2)
        x_lap, _ = x_lap.unbind(dim=-1)
    elif self.filter_name == "canny":
        x_lap = canny(x, True)
    else:
    ...
    # x_temp = canny(x,hysteresis=False)
    # # print(x_temp.min(), x_temp.max())
    # # exit()
    # x_lap = torch.cat([x_temp,x_temp, x_temp], dim=1)
    # ######### CHANGE is HERE for FILTERS USED ###########
...
class ESRGAN_EESN(nn.Module):
  def __init__(self, in_nc, out_nc, nf, nb, eesn_filter, eesn_filter_size):
    super(ESRGAN_EESN, self).__init__()
    self.netRG = RRDBNet(in_nc, out_nc, nf, nb)
    self.netE = EESN(eesn_filter, eesn_filter_size)
